...
async function buildPDF(response, dataCallback, endCallback) {
    ...
    try {
        ...
        doc.font('./fonts/calibri-bold.ttf').fontSize(13).text(`CERTIFICADO DE ASISTENCIA Y APROBACIÓN Nº ${response.curso.correlativo} - ${moment().year()}`, {
            align: 'center',
        });
...
const updateVigenciaCursos = async (req = request, res = response) => {
    const id = req.params.id;
    const curso = await updateVigenciaCursosRepository(id, req.body);
    res.json({
        curso
    });
}

const deleteCursoExternos = async (req = request, res = response) => {
    const id = req.params.id;
    const curso = await deleteCursoExternosRepository(id);
    res.json(curso);
}
