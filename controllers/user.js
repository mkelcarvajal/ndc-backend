...
const generateUserCertificate = async (req = request, res = response) => {
    try {
        ...
        const qrimage = await qr.toDataURL(`http://localhost:4200/verificarcertificados/${response.nombrecompleto}/${response.idcurso}/${response.fechafinalizacion}/${response.fechavencimiento}/${response.puestotrabajo}/${response.rut}`);
